...
После раздела с цитатой стоит галерея «Выставки R·ART в фотографиях»: кадры прошлой выставки студии о Василии Сурикове, три в ряд на компьютере и один на телефоне, со стрелками и увеличением по клику. Подбор и соответствие файлов описаны в [ИСТОЧНИКИ.md](ИСТОЧНИКИ.md).
...
Изображения и осенний ролик находятся в `assets`; онлайн-вставки используют адреса jsDelivr, закреплённые за коммитом с материалами. Шрифты встроены в HEAD. Первый экран использует первоначальный `pushkin-hero.jpg`: видео, кнопка оживления, автозапуск, параллакс и приближение портрета убраны. Цены и расписание сеансов опубликованы. Покупка билета идёт через форму Яндекс Билетов в разделе «Как посетить»: её параметры заданы в `PUSHKIN_EXHIBIT_CONFIG.tickets` в FOOTER (`clientKey`, `regionId`, `eventId`). Форма загружается при подходе к блоку покупки; если скрипт оператора недоступен, в карточке остаётся телефон и указание на кассу.
